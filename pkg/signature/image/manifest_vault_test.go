...
var _ = Describe("manifest wirh real vault", Label("e2e"), func() {
	...
	Describe("using remote Vault server", func() {
		...
		DescribeTable("sign and verify image manifest one time using per request auth with VAULT_JWT on GitHub CI",
			func(ctx SpecContext, keyRef, certRef, chainRef, rootRef string, useVerification bool) {
				fmt.Printf("CI=%v\n", os.Getenv("CI"))
				fmt.Printf("VAULT_ADDR=%v\n", os.Getenv("VAULT_ADDR") != "")
				fmt.Printf("VAULT_ROLE=%v\n", os.Getenv("VAULT_ROLE") != "")
				fmt.Printf("ACTIONS_ID_TOKEN=%v\n", os.Getenv("ACTIONS_ID_TOKEN"))
		...
		DescribeTable("sign and verify image manifest multiple times using background token refreshing with APP_ROLE",
