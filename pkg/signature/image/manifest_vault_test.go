...
var _ = Describe("manifest", Serial, func() {
	Describe("using local Vault server", func() {
		DescribeTable("sign and verify image manifest using auth method VAULT_TOKEN",
		...
		DescribeTable("sign and verify image manifest using auth method JWT",
			func(ctx SpecContext, keyType cert_utils.KeyType) {
				tmpDir := GinkgoT().TempDir()
				vaultServer := vault_server.NewVaultServer(tmpDir)
				vaultServer.Stop(ctx) // Ensure the server is stopped
				...
				GinkgoT().Setenv("TRANSIT_SECRET_ENGINE_PATH", vaultTransitPath)
				GinkgoT().Setenv("VAULT_ADDR", vaultServer.Addr.String())
				GinkgoT().Setenv("VAULT_JWT_TOKEN", jwtTokenStr)
				GinkgoT().Setenv("VAULT_JWT_ROLE", jwtRoleName)
	...
	Describe("using remote Vault server", func() {
		DescribeTable("sign and verify image manifest using auth method VAULT_TOKEN",
		...
		DescribeTable("sign and verify image manifest using auth method APP_ROLE",
