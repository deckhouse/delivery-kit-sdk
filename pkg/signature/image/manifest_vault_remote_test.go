package image_test

import (
	"fmt"
	"os"
	"time"

	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"

	"github.com/deckhouse/delivery-kit-sdk/pkg/signver"
)

var _ = Describe("manifest with real vault using remote Vault server", Label("e2e", "remote"), func() {
	DescribeTable("sign and verify image manifest one time using static auth token with VAULT_TOKEN",
		Label("vault-token"),
		func(ctx SpecContext, keyRef, certRef, chainRef, rootRef string, useVerification bool) {
			if os.Getenv("TRANSIT_SECRET_ENGINE_PATH") == "" ||
				os.Getenv("VAULT_ADDR") == "" ||
				os.Getenv("VAULT_TOKEN") == "" {
				Skip("Skipped because TRANSIT_SECRET_ENGINE_PATH, VAULT_ADDR or VAULT_TOKEN is not set")
			}

			sv, err := signver.NewSignerVerifier(ctx, certRef, chainRef, signver.KeyOpts{
				KeyRef: keyRef,
			})
			Expect(err).To(Succeed())

			manifest := testSign(ctx, sv)

			if !useVerification {
				return
			}

			testVerify(ctx, manifest, []string{rootRef})
		},
		Entry(
			"ED25519 key",
			"hashivault://dh-2025-july",
			"",
			"",
			"",
			true,
		),
		Entry(
			"ECDSA_P256 key",
			"hashivault://dh-2025-aug-ec",
			"",
			"",
			"",
			true,
		),
	)

	// ACTIONS_ID_TOKEN is a static JWT obtained from the GitHub OIDC endpoint once before the test run.
	// Its maximum lifetime is 10 minutes in GitHub Actions.
	DescribeTable("sign and verify image manifest one time using per request auth with WERF_VAULT_AUTH_JWT on GitHub CI",
		Label("static-github-jwt"),
		func(ctx SpecContext, keyRef, certRef, chainRef, rootRef string, useVerification bool) {
			if os.Getenv("CI") != "true" ||
				os.Getenv("VAULT_ADDR") == "" ||
				os.Getenv("WERF_VAULT_AUTH_ROLE") == "" ||
				os.Getenv("ACTIONS_ID_TOKEN") == "" {
				Skip("Skipped because CI=false or VAULT_ADDR, WERF_VAULT_AUTH_ROLE or ACTIONS_ID_TOKEN is not set")
			}

			// GinkgoT().Setenv("VAULT_ADDR", "")
			GinkgoT().Setenv("TRANSIT_SECRET_ENGINE_PATH", "dh-signer-dev")
			GinkgoT().Setenv("WERF_VAULT_AUTH_PATH", "github")
			GinkgoT().Setenv("WERF_VAULT_AUTH_JWT", os.Getenv("ACTIONS_ID_TOKEN"))

			sv, err := signver.NewSignerVerifier(ctx, certRef, chainRef, signver.KeyOpts{
				KeyRef: keyRef,
			})
			Expect(err).To(Succeed())

			manifest := testSign(ctx, sv)

			if !useVerification {
				return
			}

			testVerify(ctx, manifest, []string{rootRef})
		},
		Entry(
			"ED25519 key",
			"hashivault://dh-2025-aug-ec",
			"",
			"",
			"",
			true,
		),
		Entry(
			"ECDSA_P256 key",
			"hashivault://dh-2025-aug-ec",
			"",
			"",
			"",
			true,
		),
	)

	DescribeTable("sign image manifest multiple times using with APP_ROLE",
		Label("approle"),
		func(ctx SpecContext, keyRef, certRef, chainRef, rootRef string, signingExperimentTimeout, signingExperiment, signingAttemptInterval time.Duration) {
			if os.Getenv("TRANSIT_SECRET_ENGINE_PATH") == "" ||
				os.Getenv("VAULT_ADDR") == "" ||
				os.Getenv("WERF_VAULT_AUTH_ROLE_ID") == "" ||
				os.Getenv("WERF_VAULT_AUTH_SECRET_ID") == "" {
				Skip("Skipped because TRANSIT_SECRET_ENGINE_PATH, VAULT_ADDR, WERF_VAULT_AUTH_ROLE_ID or WERF_VAULT_AUTH_SECRET_ID is not set")
			}

			sv, err := signver.NewSignerVerifier(ctx, certRef, chainRef, signver.KeyOpts{
				KeyRef: keyRef,
			})
			Expect(err).To(Succeed())

			experimentStartedAt := time.Now()
			attempt := 1

			Eventually(func() time.Duration {
				attemptStartedAt := time.Now()
				fmt.Printf("Signing %d ... ", attempt)
				testSign(ctx, sv)
				fmt.Printf("done (%s, %s).\n", time.Since(attemptStartedAt), time.Now().Format(time.TimeOnly))
				attempt++
				return time.Since(experimentStartedAt)
			}, signingExperimentTimeout, signingAttemptInterval).Should(BeNumerically(">=", signingExperiment))
		},
		Entry(
			"should operate with auth token which has ttl=3m",
			"hashivault://dh-2025-aug-ec",
			"",
			"",
			"",
			time.Minute*11,
			time.Minute*10,
			time.Second*10,
		),
	)
})
