...
// FIXME(ilya-lesikov): system() call doesn't show stderr
int welf_save_elf_signature_via_objcopy(Elf *elf, const void *data, size_t data_size, const char *elf_path) {
    ...
    if (has_signature_section) {
        ...
        snprintf(rm_section_cmd, sizeof(rm_section_cmd),
                 "objcopy --remove-section %s '%s' '%s'", WERF_SIGNATURE_SECTION_NAME, elf_path, tmp_elf_cleaned);
        int ret = system(rm_section_cmd);
        if (ret != 0) {
            welf_set_errmsg("save_elf_signature_via_objcopy: objcopy remove-section failed with code %d",
                            WEXITSTATUS(ret));
            unlink(tmp_elf_note_data);
    ...
    int ret = system(add_section_cmd);
    unlink(tmp_elf_note_data);
    if (has_signature_section) unlink(tmp_elf_cleaned);
    if (ret != 0) {
        unlink(tmp_elf_final);
        welf_set_errmsg("save_elf_signature_via_objcopy: objcopy failed with code %d", WEXITSTATUS(ret));
        return -1;
    }

    char cp_cmd[1024];
    snprintf(cp_cmd, sizeof(cp_cmd), "cp '%s' '%s'", tmp_elf_final, elf_path);
    ret = system(cp_cmd);
    unlink(tmp_elf_final);
    if (ret != 0) {
        welf_set_errmsg("save_elf_signature_via_objcopy: cp failed with code %d", WEXITSTATUS(ret));
        return -1;
    }
